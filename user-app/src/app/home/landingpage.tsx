import React from "react"
import "./landingpage.css"

export default function LandingPage() {
  return (
    <div className="landingpage">
      <div className="first">
        <div className="itemf1">
          <h2>HUFLITGYM</h2>
          <p>Transform Your Body</p>
        </div>
        <button className="search">Search</button>
        <ul className="itemf2">
          <li>Home</li>
          <li>Program</li>
          <li>Coaching</li>
          <li>Membership</li>
          <li>About us</li>
        </ul>
        <button className="authencation">Login</button>
        <button className="authencation">Sign Up</button>
      </div>
      <div className="second">
        <div className="left">
          <p className="itemsl1">Achive Your</p>
          <p className="itemsl2">FITNESS GOALS</p>
          <p className="itemsl1">With FitMaker</p>
          <div className="itemsl3">
            "Join The Fitmaker Community And Transform Your Fitness Journey. Our
            Expert Coaches And <br />
            Personalized Programs Are Designed To Help You Achive Your Goals And
            Exceed Your <br />
            -32' Expectations. Ready To Make A Change?"
          </div>
          <button>Start Your Journey</button>
          <button>Explore Programs</button>
        </div>
      </div>
      <div className="third">
        <div className="itemt1">
          <div className="itemt11">
            <div>96%</div>
            <div className="tiltle">Client Satisfaction</div>
          </div>
          <div className="itemt12">
            Our Members Love Their Results And Experience
          </div>
        </div>
        <div className="item">
          <span>+5</span>
          <div className="tiltle">Years Of Experience</div>
          <div className="summary">
            Trust In Our Proven Track Record Of Transforming
          </div>
        </div>
        <div className="item">
          <span>+800</span>
          <div className="tiltle">Active Members</div>
          <div className="summary">Join Our Thriving Fitness Community</div>
        </div>
        <div className="item">
          <span>24/7</span>
          <div className="tiltle">Support Available</div>
          <div className="summary">Expert Assistance Whenever You Need It</div>
        </div>
      </div>
      <div className="four">
        <p className="four1">Our Services</p>
        <p className="four3">
          At This Part You Can Easily Access All Of Our Services. Take A Look At
          Them And Choose Ever You Want
        </p>
        <div className="four2">
          <p className="four21">LOSING WEIGHT</p>
          <p className="four22">Click To Join Our Losing Weight Plans</p>
          <p>
            Achieve Sustainable
            <br />
            Weight Loss With Our
            <br />
            Customized Programs,
            <br />
            Designed To Help You
            <br />
            Burn Fat And Built A<br />
            Healthier, Leaner Body.
            <br />
            Start Your Journey To
            <br />A Filter You Today
          </p>
          <a href="#">Learn More</a>
        </div>

        <div className="four2">
          <p className="four21">LOSING WEIGHT</p>
          <p className="four22">Click To Join Our Losing Weight Plans</p>
          <p>
            Achieve Sustainable
            <br />
            Weight Loss With Our
            <br />
            Customized Programs,
            <br />
            Designed To Help You
            <br />
            Burn Fat And Built A<br />
            Healthier, Leaner Body.
            <br />
            Start Your Journey To
            <br />A Filter You Today
          </p>
          <a href="#">Learn More</a>
        </div>

        <div className="four2">
          <p className="four21">LOSING WEIGHT</p>
          <p className="four22">Click To Join Our Losing Weight Plans</p>
          <p>
            Achieve Sustainable
            <br />
            Weight Loss With Our
            <br />
            Customized Programs,
            <br />
            Designed To Help You
            <br />
            Burn Fat And Built A<br />
            Healthier, Leaner Body.
            <br />
            Start Your Journey To
            <br />A Filter You Today
          </p>
          <a href="#">Learn More</a>
        </div>

        <div className="four2">
          <p className="four21">LOSING WEIGHT</p>
          <p className="four22">Click To Join Our Losing Weight Plans</p>
          <p>
            Achieve Sustainable
            <br />
            Weight Loss With Our
            <br />
            Customized Programs,
            <br />
            Designed To Help You
            <br />
            Burn Fat And Built A<br />
            Healthier, Leaner Body.
            <br />
            Start Your Journey To
            <br />A Filter You Today
          </p>
          <a href="#">Learn More</a>
        </div>
      </div>

      <div className="five">
        <div className="first">
          <div className="first1">Our Plans</div>
          <div className="first2">
            Select The Plan That Suits Your Fitness Goals And Let Our Expert
            Coaches Guide You Every Step Of The Way
          </div>
          <div className="first3">
            <label>
              <input type="checkbox" />
            </label>
          </div>
        </div>
        <div className="second">
          <div className="item">
            <div className="item1">Package</div>
            <div className="item2">PRO PLAN</div>
            <div className="item3">Description</div>
            <div className="item6">
              Our Pro Plan Offers Advanced Workouts And Personalized Nutrition
              Coaching To Help You Reach Your Goals Faster. Sign Up Right Now
            </div>
            <div className="item3">Features</div>
            <div className="item4">
              <ul>
                <li>Access To All Of Our Exercise Videos</li>
                <li>Progress Tracking</li>
                <li>Supportive Online Community</li>
                <li>Advanced, Personalized Workout Plans</li>
                <li>Comprehensive Nutrition Coaching</li>
                <li>Access To Advanced Workout Programs</li>
                <li>Body Composition Analysis</li>
              </ul>
            </div>
            <div className="item5">99$/USTD</div>
            <button>Choose This Plan</button>
          </div>
          <div className="item">
            <div className="item1">Package</div>
            <div className="item2">PRO PLAN</div>
            <div className="item3">Description</div>
            <div>
              Our Pro Plan Offers Advanced Workouts And Personalized Nutrition
              Coaching To Help You Reach Your Goals Faster. Sign Up Right Now
            </div>
            <div className="item3">Features</div>
            <div className="item4">
              <ul>
                <li>Access To All Of Our Exercise Videos</li>
                <li>Progress Tracking</li>
                <li>Supportive Online Community</li>
                <li>Advanced, Personalized Workout Plans</li>
                <li>Comprehensive Nutrition Coaching</li>
                <li>Access To Advanced Workout Programs</li>
                <li>Body Composition Analysis</li>
              </ul>
            </div>
            <div className="item5">99$/USTD</div>
            <button>Choose This Plan</button>
          </div>
          <div className="item">
            <div className="item1">Package</div>
            <div className="item2">BEGINNER PLAN</div>
            <div className="item3">Description</div>
            <div>
              Start Your Fitness Journey With Our Beginner Plan. Built A Strong
              Foundation With Basic Workouts And Essential
            </div>
            <div className="item3">Features</div>
            <div className="item4">
              <ul>
                <li>Access To All Of Our Exercise Videos</li>
                <li>Progress Tracking</li>
                <li>Supportive Online Community</li>
                <li>Advanced, Personalized Workout Plans</li>
                <li>Comprehensive Nutrition Coaching</li>
                <li>Access To Advanced Workout Programs</li>
                <li>Body Composition Analysis</li>
              </ul>
            </div>
            <div className="item5">99$/USTD</div>
            <button>Choose This Plan</button>
          </div>
        </div>
      </div>

      <div className="six">
        <div className="items1">
          <div className="items11">
            <div className="items111">Our Fitness </div>
            <div className="items112">Tools</div>
          </div>
          <div className="items12">
            <button>Previous</button>
            <button>Next</button>
          </div>
        </div>
        <div className="items2">
          Access A Variety Of Tools To Help You Reach Your Fitness Goals More
          Effectively
        </div>
        <div className="items3">
          <div className="items31">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmtnvTY7SelArPsbFqugw_bD9jFpmnJ0mtSA&s" />
            <a href="#">Learn More</a>
          </div>
          <div className="items31">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmtnvTY7SelArPsbFqugw_bD9jFpmnJ0mtSA&s" />
            <a href="#">Learn More</a>
          </div>
          <div className="items31">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmtnvTY7SelArPsbFqugw_bD9jFpmnJ0mtSA&s" />
            <a href="#">Learn More</a>
          </div>
          <div className="items31">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmtnvTY7SelArPsbFqugw_bD9jFpmnJ0mtSA&s" />
            <a href="#">Learn More</a>
          </div>
          <div className="items31">
            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmtnvTY7SelArPsbFqugw_bD9jFpmnJ0mtSA&s" />
            <a href="#">Learn More</a>
          </div>
        </div>
      </div>
    </div>
  )
}
